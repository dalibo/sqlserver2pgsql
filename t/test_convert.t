...
@test "schema conversion test" {
  ...
  options_to_try=( "-i" "-nr" "-num" "-keep_identifier_case" "-validate_constraints=after" )
  declare -a all_combinations=("blank ")
	
  for option_to_try in ${options_to_try[@]} ; do
		for exiting_combination in ${all_combinations[@]}; do
    	all_combinations+="${option_to_try}_sep_${exiting_combination} "
		done
  done
